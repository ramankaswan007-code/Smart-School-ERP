...
import pandas as pd
import time
from database import get_db_connection, safe_sort, get_unified_class_list, get_current_school
...
# --- मास्टर डेटा ---
all_teachers = safe_sort(pd.read_sql_query(f"SELECT Name FROM teachers WHERE school_id='{DEFAULT_SCHOOL}'", conn)['Name'].tolist())
all_subjects = safe_sort(pd.read_sql_query(f"SELECT name FROM subjects WHERE school_id='{DEFAULT_SCHOOL}'", conn)['name'].tolist())
all_classes = get_unified_class_list(DEFAULT_SCHOOL)
all_tt_data = pd.read_sql_query(f"SELECT * FROM timetable_data WHERE school_id='{DEFAULT_SCHOOL}'", conn)
...
with st.expander("✨ 🤖 AI ऑटो-पायलट (1-Click Timetable Generator)", expanded=False):
    ...
    mapping_data = pd.read_sql_query(f"SELECT * FROM subject_mapping WHERE school_id='{DEFAULT_SCHOOL}'", conn)
    ...
    if mapping_data.empty:
        ...
        if st.button("🚀 AI ऑटो-जनरेट शुरू करें (पुराना टाइम-टेबल रीसेट हो जाएगा)", type="primary"):
            with st.spinner("🧠 AI टाइम-टेबल बना रहा है... कृपया प्रतीक्षा करें (टकराव से बचते हुए)..."):
                # 1. डेटा साफ़ करें
                conn.execute(f"DELETE FROM timetable_data WHERE school_id='{DEFAULT_SCHOOL}'")
                
                slots = pd.read_sql_query(f"SELECT period_name FROM time_slots WHERE school_id='{DEFAULT_SCHOOL}' AND period_name NOT IN ('प्रार्थना सभा', 'मध्यांतर')", conn)['period_name'].tolist()
...
if main_selection:
    ...
    all_slots = pd.read_sql_query(f"SELECT period_name FROM time_slots WHERE school_id='{DEFAULT_SCHOOL}' ORDER BY slot_id ASC", conn)['period_name'].tolist()
    ...
    for p_name in all_slots:
        ...
        if r_cols[0].button(f"⚡ {p_name}\n(सभी 6 दिन)", key=f"all_{p_name}", type="primary", use_container_width=True):
            if s_tool == "ERASER":
                conn.execute(f"DELETE FROM timetable_data WHERE school_id='{DEFAULT_SCHOOL}' AND period='{p_name}' AND {'unified_class' if 'कक्षा' in editor_mode else 'teacher'}='{main_selection}'")
        ...
        for i, d in enumerate(days):
            ...
            if r_cols[i+1].button(btn_txt, key=f"c_{p_name}_{d}", disabled=is_disabled, use_container_width=True):
                if s_tool == "ERASER":
                    conn.execute(f"DELETE FROM timetable_data WHERE school_id='{DEFAULT_SCHOOL}' AND day='{d}' AND period='{p_name}' AND {'unified_class' if 'कक्षा' in editor_mode else 'teacher'}='{main_selection}'")
                elif t_tool and s_tool:
                    conn.execute("INSERT OR REPLACE INTO timetable_data VALUES (?,?,?,?,?,?)", (DEFAULT_SCHOOL, main_selection if "कक्षा" in editor_mode else t_tool, d, p_name, t_tool if "कक्षा" in editor_mode else main_selection, s_tool))
                conn.commit(); st.rerun()
